...
main( int argc, char *argv[] )
{
    ...
    sprintf(str, "prion-data-model-6-tloss-var-0-to-2-v%d.txt" ,name_id);
    double Vscale = 1;
    FILE *ff = fopen( str, "w");
	...
	for(sim = 0; sim<N_sims; sim++)
		...
		for (i = 0; i<max_aggregate_size; i++) //aggregate initial condition
            ...
            random_number = (ran1(&seed));
            lambda = 1.75*( 0 + 2*random_number );
            y[ (int)(lambda*50/log(2)*6*Volume*Vscale) ] = 1;
		...
		for (i = 0; i < N_steps - 1; i++)
            ...
            predicted_average = 1.75*tau_c/log(2)*1;
            t_trunc = 0;
            for( j = 0; j<98; j++)
            {
