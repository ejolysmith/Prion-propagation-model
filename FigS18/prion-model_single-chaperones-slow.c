...
main( int argc, char *argv[] )
{
    ...
    sprintf(str, "prion-data-single-chaperones-slow-long-v%d.txt" ,name_id);
    FILE *ff = fopen( str, "w");
            ...
            while(sim < 1)
                ...
                for (i = 0; i<max_aggregate_size; i++) //aggregate initial condition
                    ...
                    else
                    {
                        ...
                        if (t_tot > wait_time && t_tot < wait_time + 1000)
                        {
                            
                            fprintf(ff, "%lf %lf %d %d %lf \n",  t_tot - wait_time , ((float)(c))/Volume, is_it_loss, z, t );
