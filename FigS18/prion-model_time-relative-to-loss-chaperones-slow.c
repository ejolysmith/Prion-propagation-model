...
long double virtual_chaperone_production_rate( double Volume, int z ){  //This is added to keep the total rate more or less constant so that there is an equal number of values saved throughout the trajectory (values are saved at each reaction)
...
main( int argc, char *argv[] )
{
    ...
    sprintf(str, "prion-data-model-6-time-rel-slow-v%d.txt" ,name_id);
    FILE *ff = fopen( str, "w");
    ...
    for(run = 0; run < N_parameters; run++)
    {
        ...
        gammma = 1.12200000e-03;
        
        random_number = (ran1(&seed));
        alpha = 1.97000000e-04;
