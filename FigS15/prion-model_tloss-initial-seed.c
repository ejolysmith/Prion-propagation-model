...
float seed_factor = 2; //When this is set to 1, the initial seed is chosen as described in SI 3.2.2.2.
//That is the initial seed size used throughout the manuscript.
//When seed_factor = 2, then an initial seed size bigger by a factor of two is used.
//When seed_factor = 0.5, then an initial seed size smaller by a factor of 0.5 is used.
...
double Vscale = 1;  //This is used for Fig. S9e. When not set to 1 it changes the size of the cells.


//The reactions definining the model:

long double prion_production_rate(double Volume){
    return lambda*Volume*Vscale;
}
...
long double total_elongation_rate( int x, int Y_tot, double Volume ){
    return gammma*x*Y_tot/Volume/Vscale;
}
...
main( int argc, char *argv[] )
{
    ...
    char str[55];
    sprintf(str, "prion-data-model-tloss-lam1p75-uni-v%d-large-seed.txt" ,name_id);   //Where the data is saved
    double Vscale = 1;
    FILE *ff = fopen( str, "w");
    ...
    for(run = 0; run < N_parameters; run++)
    {
        ...
        //The five curves in Fig.5d correspond to changing to the pairs of parameters bellow.
	...
	for(sim = 0; sim<N_sims; sim++)
		...
		for (i = 0; i<max_aggregate_size; i++) //aggregate initial condition
            ...
            y[ (int)(126*6*Volume*Vscale*seed_factor) ] = 1;
