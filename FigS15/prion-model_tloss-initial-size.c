...
main( int argc, char *argv[] )
{
    ...
    char str[50];
    sprintf(str, "prion-data-model-tloss-lam1p75-uni-v%d.txt" ,name_id);   //Where the data is saved
    double Vscale = 1;
    FILE *ff = fopen( str, "w");
	...
	for(sim = 0; sim<N_sims; sim++)
		{
            ...
            //comment and uncomment to pick initial volume distribution
            Volume = 1 + random_number;   //Uniform
            
            //random_number = 0.5*log(2)*log(2)*( 1 + (ran1(&seed)));  //Exponential
