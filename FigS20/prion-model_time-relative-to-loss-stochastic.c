...
double rate_total_function(int x, int Y_tot, int y0, double Volume, int L){
    return (prion_production_rate(2,20) + prion_degradation_rate(x) + total_elongation_rate(x,Y_tot, 1) + total_aggregate_degradation_rate(Y_tot)+ 0*L_production_rate() + 0*L_degradation_rate(L) + 10);
}
...
main( int argc, char *argv[] )
{
    ...
    sprintf(str, "prion-data-model-6-time-rel-L-0p1-v%d.txt" ,name_id);
    FILE *ff = fopen( str, "w");
                ...
                for (i = 0; i<max_aggregate_size; i++) //aggregate initial condition
                    ...
                    else
                    {
                        ...
                        if (t_tot > wait_time && Y_tot > 0)
                        {
                            ...
                            fprintf(ff, "%lf %lf %d %lf \n", ((double)(fluorescence))/Volume    ,  t_tot , is_it_loss, ((double)(L))/Lbar );
